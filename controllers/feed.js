...
exports.getPosts = async (req, res, next) => {
  ...
  try {
    ...
    const posts = await Post.find().sort({ createdAt: -1 }).skip((page - 1) * ITEMS_PER_PAGE)
      .limit(ITEMS_PER_PAGE).populate('creator');
...
exports.createPost = async (req, res, next) => {
  ...
  try {
    ...
    await user.save();
    io.getIO().emit('posts', { action: 'create', post: { ...post._doc, creator: { _id: creator, name: user.name } } })
...
exports.editPost = async (req, res, next) => {
  ...
  try {
    ...
    post.imageUrl =
      req.file ? req.file.path.replace(/\\/g, '/') : req.body.image;
